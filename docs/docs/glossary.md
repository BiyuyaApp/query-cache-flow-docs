...
title: Glossary
description: Key terms and definitions used in CACHE-FLOW documentation
---

# Glossary

This glossary defines key terms used throughout the CACHE-FLOW documentation.

## Core Concepts

### Cache Key
A unique identifier used by TanStack Query to store and retrieve cached data. In CACHE-FLOW, cache keys are structured objects with `entity`, `method`, and optionally `id` properties.
...
### QueryKey Type
The TypeScript type that defines the structure of all CACHE-FLOW cache keys:
...
### Invalidate
To mark cached data as stale, triggering a refetch when the query is next accessed. CACHE-FLOW provides `invalidateQueriesForKeys()` for batch invalidation.
...
### Cancel
To abort in-flight queries, typically done before optimistic updates to prevent race conditions. CACHE-FLOW provides `cancelQueriesForKeys()`.
...
### Wrapper Hook
A thin function that wraps a KUBB-generated hook, injecting CACHE-FLOW query keys and invalidation logic. This is the main integration point between KUBB and CACHE-FLOW.
...
### QueryClient
The TanStack Query client that manages all query caching, invalidation, and refetching. CACHE-FLOW utilities use the QueryClient internally.

## Tools & Libraries

### KUBB
A code generator that reads OpenAPI specifications and produces TypeScript types and React Query hooks. CACHE-FLOW wraps KUBB-generated hooks with cache key management.

### OpenAPI / Swagger
A specification for describing REST APIs. KUBB reads OpenAPI specs to generate type-safe code.

### TanStack Query (React Query)
The underlying data fetching and caching library that CACHE-FLOW builds upon. Provides `useQuery`, `useMutation`, and the QueryClient.
...
- [Introduction](./intro.md) - Overview of CACHE-FLOW
